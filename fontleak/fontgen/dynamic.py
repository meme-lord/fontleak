...
for code_point in range(256, 0x1ffff):
    ...
    if ('LATIN ' in desc or 'MATHEMATICAL ' in desc) and not 'ARABIC' in desc and not 'GREEK' in desc and not 'CYRILLIC' in desc and not 'HEBREW' in desc and not 'HIRAGANA' in desc and not 'KATAKANA' in desc and not 'HANGUL' in desc and not 'THAI' in desc:
...
def create_glyph(glyph_name, unicode_value, horiz_adv_x, path_data):
    ...
    return '<glyph glyph-name="{}" unicode="&#x{:04X};" horiz-adv-x="{}" d="{}"/>'.format(
...
def generate_custom_glyphs(alphabet):
    ...
    for i in range(alphabet_len):
        ...
        glyphs.append(create_glyph(glyph_name, 0xF0000 + current_unicode, horiz_adv_x, path_data))
        leak_glyphs.append(glyph_name)
        current_unicode += 1
    
    # 2. Add lu with width of alphabet_len + 2
    glyphs.append(create_glyph('lu', 0xF0000 + current_unicode, alphabet_len + 1, f"M{current_unicode} 0z"))
    current_unicode += 1
...
def generate_feature_file(unknown_glyphs, char_glyphs, leak_glyphs, index_glyphs, idx_max, output_file='mylig.fea'):
...
def generate_font(output_font='myfont.svg', output_feature='mylig.fea', alphabet="0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ-_", idx_max=2400):
