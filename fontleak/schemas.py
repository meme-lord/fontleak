...
class BaseLeakParams(BaseModel):
    selector: str = Field(
        default=os.getenv("SELECTOR", "script:first-of-type"),
        description="CSS selector for target element"
    )
    parent: str = Field(
        default=os.getenv("PARENT", "body"),
    ...
    attr: str = Field(
        default=os.getenv("ATTR", "textContent"),
        description="Attribute to exfiltrate"
    )
...
class StaticParams(BaseLeakParams):
    length: int = Field(
        default=int(os.getenv("LENGTH", 100)),
        description="Length of the payload"
    )
    browser: str = Field(
